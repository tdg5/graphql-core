        ...
        try:
            ...
            if self.is_awaitable(result):
                ...
                async def await_result() -> Any:
                    try:
                        last_time = time.perf_counter()
                        print(f"11a: {last_time} : {last_time - start_time}", flush=True)
                        completed = self.complete_value(
                            return_type, field_nodes, info, path, await result
                        )
                        last_time = time.perf_counter()
                        print(f"11b: {last_time} : {last_time - start_time}", flush=True)
                        if self.is_awaitable(completed):
                            last_time = time.perf_counter()
                            print(f"11c: {last_time} : {last_time - start_time}", flush=True)
                            return await completed
                        last_time = time.perf_counter()
                        print(f"11d: {last_time} : {last_time - start_time}", flush=True)
                        return completed
