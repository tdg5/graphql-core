    ...
    def execute_field(
        ...
        try:
            ...
            if self.is_awaitable(result):
                # noinspection PyShadowingNames
                # new_time = time.perf_counter()
                # print(f"11: {new_time} : {new_time - last_time}", flush=True)
                # last_time = new_time
                fun_time = last_time
            ...
            if self.is_awaitable(completed):
                ...
                async def await_completed() -> Any:
                    ...
                    try:
                        new_time = time.perf_counter()
                        print(f"14a: {new_time} : {new_time - last_time}", flush=True)
                        last_time = new_time
                        thing = await completed
                        new_time = time.perf_counter()
                        print(f"14b: {new_time} : {new_time - last_time}", flush=True)
                        last_time = new_time
                        return thing
                    except Exception as raw_error:
                        new_time = time.perf_counter()
                        print(f"14c: {new_time} : {new_time - last_time}", flush=True)
                        last_time = new_time
                        error = located_error(raw_error, field_nodes, path.as_list())
                        self.handle_field_error(error, return_type)
                        return None
